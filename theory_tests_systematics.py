...
PLOTTING_DIR = Path('./theory_tests_kids_camb_no_systematics')
...
for theory_test_index in range(10):
    ...
    # %%
    shift_nz = False # Whether to shift the n(z) using the dz biases and covariance
    ...
    # %%
    rng = np.random.default_rng(seed=sim_num)
    np.random.seed(100)
    omega_k = 0.0
    ...
    if SIMULATOR_TYPE == 'glass':
        ...
        # z_dense = np.linspace(zmin, zmax, 2000)
        # chi_dense = np.array([cosmo.comoving_distance(z) for z in z_dense])
        # z_of_chi = interp1d(chi_dense, z_dense, kind='cubic')
        # chi_uniform = np.arange(chi_dense[0], chi_dense[-1], dx)
        # zb = z_of_chi(chi_uniform)
        zb = glass.shells.distance_grid(cosmo, zmin, zmax, dx=dx)
    ...
    def process_cls(mask, nbins, nside, alm, alm_rand, lower_lscale, upper_lscale, nbands):
    ...
    realised_unmixed_shear_cls, cll_bands, bandpowers = process_cls(mask, nbins, nside, alm, alm_rand, lower_lscale, upper_lscale, nbands, )
    ...
    plt.savefig(out_dir / f'mixed_sim{sim_num}.png', dpi=300)
    plt.close()
    fig, ax = plt.subplots(nbins, nbins, figsize=(15,15))
    ...
    plt.tight_layout()
    plt.savefig(out_dir / f'mixed_sim{sim_num}_bandpowers_ratios.png', dpi=300)
    plt.close()
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    plt.savefig(out_dir / f'unmixed_sim{sim_num}.png', dpi=300)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(out_dir / f'unmixed_sim{sim_num}_bandpowers_ratios.png', dpi=300)
    plt.close()
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    plt.savefig(out_dir / f'shear_cls_sim{sim_num}.png', dpi=300)
    plt.close()